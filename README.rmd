...
Our two main questions were whether or not the elevation of a tree will affect growth while accounting for different tree diameters and whether or not the logged soil nitrogen levels affect a tree's grwoth accounting for each tree's starting diameter. We decide to draw our conclusion from the spatial models over the OLS models since we can consider spatial correlation and have better general fit to the data. 

Elevation seems to have a negative statistically significant relationship with growth while controlling for tree diameter. This relationship is stronger among pin trees compared to bur trees. There is weak eveidence that soil nitrogen has an effect on growth while accounting for starting diameters. Among Pin trees, soil nitrogen has a weak relationship that is weakly statistically significant. Among Bur trees, soil nitrogen has a statistically insignificant relationship with growth. 
